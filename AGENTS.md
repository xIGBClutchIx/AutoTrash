...
- Agents may read:
  ...
  - Dev server files under `devserver/` for reference only.
- Agents may modify:
  - Project files under the repository root, excluding runtime/cache/output folders.
- Agents must never:
  - Modify files under `devserver/`.
...
- Local dev server files are provisioned by the Scaffoldit Hytale Gradle plugin.
- Dev server files live under `devserver/`.
- Use `./gradlew setupServer` to provision/recreate the dev server.
- Use `./gradlew runServer` (or `./gradlew devServer`) to run it.
