...
/**
 * Toggles auto-trash notifications.
 */
public final class TrashNotifyCommand extends CommandBase {
     ...
     */
    public TrashNotifyCommand(@NonNullDecl ComponentType<EntityStore, AutoTrashPlayerSettings> settingsComponentType) {
        super("notify", "Enable or disable auto-trash notifications.");
    ...
    protected void executeSync(@NonNullDecl CommandContext context) {
        ...
        if (!context.provided(enabledArg)) {
            context.sendMessage(Message.raw("AutoTrash notifications are " + (settings.isNotify() ? "enabled" : "disabled") + "."));
            return;
        }

        boolean enabled = context.get(enabledArg);
        settings.setNotify(enabled);
        context.sendMessage(Message.raw("AutoTrash notifications are now " + (enabled ? "enabled" : "disabled") + "."));
    }
}
